...
contract VaultLogic {
  ...
  function viewPassword() external view returns (bytes32) {
    return password;
  }
  event checkPassword (bytes32 pwd, bytes32 pwd2);
  function changeOwner(bytes32 _password, address newOwner) public {
    emit checkPassword(_password, password);
